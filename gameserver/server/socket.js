...
module.exports = function(server,game,chat) {
    ...
    function onConnection(socket) {
        ...
        ++totalConnections;
        console.log(getCurrentTime(), 'Connection from %s (%d) total %d', ip, ipConnections + 1, totalConnections);
	    socket.on('pong', function(data){
            //console.log("Pong received from client", data);
        });
        socket.on('disconnect', function() {
            console.log(getCurrentTime(), 'Socket from %s has disconnected', ip);
        ...
        setTimeout(function(){
            if (!socket.hasJoined) {
                console.warn(getCurrentTime(), 'Socket from %s did not join in 6 seconds, disconnecting', ip);
