...
exports.addJackpot = function(username, index, bonus, callback) {
    getClient(function(client, callback) {        
            client.query('UPDATE users SET balance_satoshis = balance_satoshis + $1 WHERE username = $2',
              [bonus * 0.9, username[index]], callback);
            if (username.length > 1) {
                let us = username.map(s => "\"" + s.replace(/[\""]/g, '\\"') + "\"");
                us.push('""');
                us = "(" + us.join(',') + ")";
                
                client.query('UPDATE users SET balance_satoshis = balance_satoshis + $1 WHERE username != $2 and username in ' + us,
                [bonus * 0.1 / (username.length - 1), username[index]], callback);
            }
    }, callback);
}
...
exports.getBankroll = function(callback) {
    query('SELECT (' +
        ...
        function(err, results) {
            ...
            var profit = results.rows[0].profit - 200e8;
