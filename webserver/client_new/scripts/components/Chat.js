define([
    ...
    function getState() {
      console.log('55555555555', Engine.balanceSatoshis);
      ...
      state.balanceBitsFormatted = Clib.formatSatoshis(Engine.balanceSatoshis); 
      return state;
    }
    ...
    var ChatMessageClass = React.createClass({
      ...
      render: function() {
        ...
        //   var timeString = msgDate.getHours() + ':' + ((msgDate.getMinutes() < 10 )? ('0' + msgDate.getMinutes()) : msgDate.getMinutes()) + ' ';
          var timeString = msgDate.getFullYear()+'/'+(msgDate.getMonth()+1)+'/'+msgDate.getDate()+'-'+msgDate.getHours() + ':' + ((msgDate.getMinutes() < 10 )? ('0' + msgDate.getMinutes()) : msgDate.getMinutes()) + ' ';
          ...
          return D.li({ className: pri },                    
                ...
                //      }
                //    }),
                    D.div({className: 'msg-item'},
                        D.div({className: 'badge'},
                            D.a({
                                href: '/user/' + message.username,
                                target: '_blank',
                                className: 'badge-txt'
                        ...
                        D.div({className: 'msg-description'},
                            D.a({ href: '/user/' + message.username, target: '_blank'}, message.username),                           
                            D.div({ className: 'msg-body', 
                                dangerouslySetInnerHTML: { __html: Autolinker.link(escapeHTML(message.message),
                                    { truncate: 50, replaceFn: replaceUsernameMentions }
                                        )
                                    }
                                }), 
                                D.div(null, D.span({ className: 'time-stamp'}, timeString),
                            )
                        )
                    )
                 );
    ...
    return React.createClass({
        ...
        componentDidMount: function() {
            ...
            this.setState({balanceBitsFormatted: Clib.formatSatoshis(Engine.balanceSatoshis)});
        ...
        render: function() {
            ...
            switch (state.connectionState) {
                ...
                case 'JOINED':
                    ...
                    console.log('YYYYYYYYYYYYYYYYYY', state.balanceBitsFormatted);
                    //If user is logged
                    if (state.username) {
                        // If user have money
                        if (Number(state.balanceBitsFormatted) > 0) {
                            chatInputPlaceholder = 'Type here...';
                            chatInputOnKeyDown = this._sendMessage;    
                        } else {
                            chatInputPlaceholder = 'Make your first deposit to unlock the chat!';
                            chatInputDisabled = true;
                        }

                    //If user is not logged
                    } else {
                        chatInputPlaceholder = 'You have to be logged in to send message';
                        chatInputDisabled = true;
                    }
