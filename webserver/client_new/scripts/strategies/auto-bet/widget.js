...
define([
    ...
    return React.createClass({
        ...
        render: function() {
            return D.div({ className: 'value-area' },
                D.div({ className: 'half-area' },
                    D.span({ className: 'bet-title' }, 'Base Bet: '),
                    D.input({ type: 'text', className:'cash-value', ref: 'bet_amount', onChange: this.updateBetAmount, value: this.state.baseBet, disabled: this.state.active }),
                    // D.span(null, '$USD')
                ),
                D.div({ className: 'half-area' },
                    D.span({ className: 'bet-title' }, 'Auto Cashout at:'),
                    D.input({ type: 'text', className: 'cash-value', ref: 'auto_cash_at', onChange: this.updateAutoCashAt, value: this.state.autoCashAt, disabled: this.state.active }),
                    // D.span(null, 'x')
                ),
                D.div({ className: 'stra-max-bet-stop' },
                    // D.span({ className: 'bet-title' }, 'Stop if bet is > '),
                    D.input({ type: 'hidden', ref: 'max_bet_stop', onChange: this.updateMaxBetStop, value: this.state.maxBetStop, disabled: this.state.active }),
                    // D.span(null, '$USD')
                ),
