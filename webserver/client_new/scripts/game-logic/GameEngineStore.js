...
//console.error("Appconstants:" , AppConstants.Engine.HOST);
	self.hosturl = AppConstants.Engine.HOST;
        self.ws = io(AppConstants.Engine.HOST, {pingInterval: 10000, pingTimeout:5000});
         ...
         */
        setInterval(() => {
            self.ws.emit("TestDataSending", {});
        }, 4000);
	self.ws.on('ping', function(data) {
		console.log("Pingpong data:", data);
		self.ws.emit("pong", {beat: 1});
	});
         ...
         */
        self.ws.on('updated_jackpot_success', function(data) {
            console.log("Game Engine Store", data);
            if (data.name == self.username)
                self.balanceSatoshis += Number(data.v);
        });
        ...
        self.ws.on('game_starting', function(info) {
            // $('.max-profit').css('opacity', '1');
            // $('.history-bar').css('opacity', '1');
            self.playerInfo = {};
            self.joined = [];
        ...
        self.ws.on('connect', function() {
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', {
                    ...
                    if (err) {
                        console.error('Error when joining the game...', err, self.hosturl);
                        return;
                    }
        ...
        self.ws.on('jackpot_run_starting', function(s) {
            console.log("socket on called");
            self.gameState = 'jackpot_run_starting';
            self.jackpotState = s.s;
            self.jackpotCustomData = s.names;
            self.trigger('jackpot_run_starting');
            if ((s.s == 14) && s.names.pValue) {
                if (s.names.pValue.name.indexOf(self.username) !== -1) {
                    if (s.names.pValue.name[s.names.pValue.index] == self.username)
                        self.balanceSatoshis += s.names.pValue.v * 0.9;
                    else
                        self.balanceSatoshis += s.names.pValue.v * 0.1 / (s.names.pValue.name.length - 1);
                }
            } else if (s.s == 99) {
                for (let i in s.names) {
                    if (s.names[i][0] == self.username) {
                        self.balanceSatoshis += s.names[i][2]; break;
                    }
                }
            }
        });
