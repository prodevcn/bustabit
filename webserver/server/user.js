...
exports.register  = function(req, res, next) {
    ...
    database.createUser(username, password, email, ipAddress, userAgent, fp, function(err, sessionId) {
        ...
        // var result = payus.getAddressBalance({coin:'BTC', address: '13mNf2rNDcVMhNZehRpKJbyLLc3ftpsy3z'});
        // result.then(res => {
        //     console.log(res);
        // }).catch(err=>{
        //     console.error(err);
        ...
        res.render('register_confirmation', {
            user: { username, balance_satoshis: 0 },
            username: username,
            password: password,
            email: email
        })
...
exports.login = function(req, res, next) {
    ...
    database.validateUser(username, password, otp, function(err, userId, coinAddress) {
        if (err) {
        ...
        database.createSession(userId, coinAddress, ipAddress, userAgent, remember, fp, function(err, sessionId, expires) {
            if (err)
...
exports.giveawayRequest = function(req, res, next) {
    ...
    database.addGiveaway(user.id, req.ip, function(err) {
        if (err) {
...
exports.resetPassword = function(req, res, next) {
    ...
    database.validateUser(user.username, password, otp, function(err, userId) {
        if (err) {
            if (err  === 'WRONG_PASSWORD') return  res.redirect('/update-password?err=Wrong%20password.');
            if (err === 'INVALID_OTP') return res.redirect('/update-password?err=Invalid%20one-time%20password.');
        ...
        database.changeUserPassword(user.id, newPassword, function(err) {
            ...
            database.expireSessionsByUserId(user.id, function(err) {
                ...
                database.createSession(user.id, ipAddress, userAgent, false, null, function(err, sessionId) {
                    ...
                    res.cookie('id', sessionId, sessionOptions);
                    res.redirect('/security?m=Password%20changed.');
                });
            });
        });
    });
...
exports.twoFactor = function(req, res) {
    var user = req.user;
    assert(user);
    ...
    res.render('two-factor', { user: user });
};
...
* GET
* Restricted API
* Shows the change password page of the users account
**/
exports.updatePassword = function(req, res) {
    var user = req.user;
    assert(user);

    res.render('update_password', { user: user });
};
...
exports.bonus = function(req, res) {
    ...
    res.render('bonus', { user: user });
};
exports.bankroll = function(req, res) {
    var user = req.user;
    assert(user);
    res.render('bankroll', {user: user});
}
/**
 * POST
 * Restricted API
...
exports.validateResetPassword = function(req, res, next) {
    ...
    if (!recoverId || !lib.isUUIDv4(recoverId))
        return next('Invalid recovery ix');
...
exports.deposit = function(req, res, next) {
    ...
    database.getDeposits(user.id, function(err, deposits) {
        ...
        database.getCoinAddresses(user.id, function(err, coinAddresses) {
            if (err) {
                return next(new Error('Unable to get CoinAddresses: \n' + err));
            }
            user.coin_addresses = coinAddresses;
            user.deposit_address = JSON.parse(coinAddresses)['BTC'];
    ...
    database.getDeposits(user.id, function(err, deposits) {
        ...
        database.getCoinAddresses(user.id, function(err, coinAddresses) {
            ...
            database.getWithdrawals(user.id, function(err, withdrawals) {
                ...
                res.render('transactions', { user: user });
            });
        });
    });
};
...
exports.handleWithdrawRequest = function(req, res, next) {
    ...
    var amount = lib.removeNullsAndTrim(req.body.amount);
    var coin_code = lib.removeNullsAndTrim(req.body.code); // this is added
    // console.log('this is coin code+++++++++++++++++', coin_code); // this is added 
    ...
    // amount = Math.round(parseFloat(amount) * 100); //hahahahahahahahaha
    // assert(Number.isFinite(amount));

    // var minWithdraw = config.MINING_FEE + 10000;
    // var minWithdraw = (config.MINING_FEE + 10000) / 100;
    ...
    database.validateUser(user.username, password, otp, function(err) {
        ...
        if (err) {
            ...
            if (err === 'INVALID_OTP')
                ...
                return res.render('withdraw-request', { user: user, id: uuid.v4(), warning: 'Withdraw failed' });
            //Should be an user
            return next(new Error('Unable to validate user handling withdrawal: \n' + err));
        }

        withdraw(req.user.id, amount, destination, withdrawalId, fp, coin_code, function(err) {
            if (err) {
...
exports.jackpothistory = function(req, res, next) {
    // console.log('---------------uid', uuid.v4());
    var fs = require('fs');
    fs.writeFile("OK.txt", 'test', function() {})
    fs.readFile('../../gameserver/jackpothistory.txt', 'utf8', function(err, data){
        if (err) {
            return res.render('jackpothistory', { content : [] });
        }
        try {
            data = JSON.parse(data);
        } catch (e) {data = [];}        
        var getCurrentTime = function(s) {
            let make2 = t => t < 10 ? "0" + t : t;
            let nowTime = new Date(new Date(s) - new Date().getTimezoneOffset() * 60000);
            return "[" + nowTime.getFullYear() + "-" + make2(nowTime.getMonth() + 1) + "-" +  make2(nowTime.getDate()) + " " 
            + make2(nowTime.getHours()) + ":" + make2(nowTime.getMinutes()) + ":" + make2(nowTime.getSeconds()) + "] ";
        };
        res.render('jackpothistory', { content : data,  getCurrentTime});
    });
