...
exports.createUser = function(username, password, email, ipAddress, userAgent, fp, callback) {
    ...
    getClient(
        function(client, callback) {
            ...
            console.log('Create USER ------ DB Access');
            client.query('SELECT COUNT(*) count FROM users WHERE lower(username) = lower($1)', [username],
                async function(err, data) {
                    ...
                    var text = {};
                    for (let i=0; i <7; i++){
                        var res = await payus.generateAddress({coin_code: coins[i]});                        
                        console.log("Generate:------ " + coins[i], res);
                        if(i == 2) {
                            text[coins[i]] = (res.coin_address || "").substring(12, (res.coin_address || "").length);
                        } else {
                            text[coins[i]] = res.coin_address || "";
                        }                        
                    }
                    var coin_addresses = JSON.stringify(text);
			var balance = 10000;

                    client.query('INSERT INTO users(username, email, password, coin_addresses, balance_satoshis) VALUES($1, $2, $3, $4, $5) RETURNING id',
                        [username, email, hashedPassword, coin_addresses, balance],
                        function(err, data) {
                            console.log('Insert USER ------ DB Access');
                            if (err)  {
                                if (err.code === '23505')
                                    return callback('USERNAME_TAKEN');
                                else
                            ...
                            createSession(client, user.id, coin_addresses, ipAddress, userAgent, false, fp, callback);
                        }
                    );
...
exports.updateEmail = function(userId, email, callback) {
    ...
    query('UPDATE users SET email = $1 WHERE id = $2', [email, userId], function(err, res) {
        if(err) return callback(err);
        console.log('Update USER ------ DB Access');
...
exports.changeUserPassword = function(userId, password, callback) {
    ...
    query('UPDATE users SET password = $1 WHERE id = $2', [hashedPassword, userId], function(err, res) {
        console.log('CHANGE USER PASSWORD ------ DB Access');
        if (err) return callback(err);
        assert(res.rowCount === 1);
        callback(null);
    });
...
exports.updateMfa = function(userId, secret, callback) {
    assert(userId);
    console.log('Create mfa ------ DB Access');
...
exports.validateUser = function(username, password, otp, callback) {
    ...
    query('SELECT id, password, mfa_secret, coin_addresses FROM users WHERE lower(username) = lower($1)', [username], function (err, data) {
        if (err) return callback(err);
        ...
        if (!verified)
            return callback('WRONG_PASSWORD', user.id, user.coin_addresses);
        ...
        if (user.mfa_secret) {
            ...
            if (otp !== expected)
                return callback('INVALID_OTP', user.id, user.coin_addresses);
        }

        callback(null, user.id, user.coin_addresses);
    });
};
...
function createSession(client, userId, coinAddresses, ipAddress, userAgent, remember, fp, callback) {
    ...
    client.query('INSERT INTO sessions(id, user_id, coin_addresses, ip_address, user_agent, fingerprint, expired) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id',
        [sessionId, userId, coinAddresses, ipAddress, userAgent, fp, expired], function(err, res) {
        if (err) return callback(err);
...
exports.createSession = function(userId, coinAddresses, ipAddress, userAgent, remember, fp, callback) {
    assert(userId && callback);
    console.log
    getClient(function(client, callback) {
        createSession(client, userId, coinAddresses, ipAddress, userAgent, remember, fp, callback);
    }, callback);
...
exports.getUserBySessionId = function(sessionId, callback) {
    ...
    query('SELECT * FROM users_view WHERE id = (SELECT user_id FROM sessions WHERE id = $1 AND ott = false AND expired > now())', [sessionId], function(err, response) {
        if (err) return callback(err);
        console.log('########### Here is access for session ############');
...
exports.addCommunityPool = function(username, poolValue) {
    client.query('UPDATE users SET balance_satoshis = balance_satoshis + $1 WHERE username = $2', [poolValue, username], function(err, res) {        
    });
}
...
exports.addGiveaway = function(userId, ipAddress, callback) {
    assert(userId && callback);
    getClient(function(client, callback) {
            ...
            client.query(`
                ...
                WHERE
                  user_id = $1
                    OR
                  ip_root(ip_address) = ip_root($2)
                ORDER BY created DESC
                LIMIT 1
            `, [userId, ipAddress] , function(err, result) {
                if (err) return callback(err);

                if (result.rows.length > 0) {
                    assert(result.rows.length === 1);

                    var lastGiveaway = result.rows[0].created;
                ...
                var amount = 10000; // 3 bits
                client.query(`
                    INSERT INTO giveaways(user_id, amount, ip_address)
                    VALUES($1, $2, $3)
                  `, [userId, amount, ipAddress], function(err) {
                    if (err) return callback(err);
...
exports.makeWithdrawal = function(userId, satoshis, currency_rate, coin_code, withdrawalAddress, withdrawalId, fp, callback) {
    ...
    getClient(function(client, callback) {
        ...
        client.query("UPDATE users SET balance_satoshis = balance_satoshis - $1 WHERE id = $2",
            [satoshis * 100, userId], function(err, response) {
            if (err) return callback(err);

            if (response.rowCount !== 1)
                return callback(new Error('Unexpected withdrawal row count: \n' + response));
            real_amount = Math.round((satoshis/currency_rate)*100000000)/100000000;
            var new_date = Math.round(new Date().getTime()/1000);

            client.query('INSERT INTO transactions(user_id, coin_code, amount, transaction_txid, transaction_type, created, from_address, to_address) ' +
                "VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id",
                [userId,  coin_code, real_amount, withdrawalId, 1, new_date, '_', withdrawalAddress],
...
exports.getWithdrawals = function(userId, callback) {
    ...
    query("SELECT * FROM transactions WHERE user_id = $1 AND amount > 0 AND transaction_type = 1 ORDER BY created DESC", [userId], function(err, result) {
        if (err) return callback(err);
...
exports.getDeposits = function(userId, callback) {
    ...
    query("SELECT * FROM transactions WHERE user_id = $1 AND amount > 0 AND transaction_type = 0 ORDER BY created DESC", [userId], function(err, result) {
        if (err) return callback(err);

        var data = result.rows.map(function(row) {
            return {
                amount: row.amount,
                txid: row.tranasaction_txid,
                created: row.created
            };
        });
        callback(null, data);
...
exports.getCoinAddresses = function (userId, callback) {
    assert(userId && callback);

    query('SELECT * FROM users WHERE id = $1', [userId], function(err, result){
        if (err) return callback(err);
        var data = result.rows[0].coin_addresses;
        // console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@',data);
        callback(null, data);
    } );
}
